...
# 3. Generate 10-digit random number for MDM_CHALLENGE_API_KEY
echo "🔑 Generating API key..."
RANDOM_API_KEY=$(shuf -i 1000000000-9999999999 -n 1)
sed -i.bak "s/MDM_CHALLENGE_API_KEY=\"your_api_key_here\"/MDM_CHALLENGE_API_KEY=\"$RANDOM_API_KEY\"/" .env
rm .env.bak
echo "✅ Set MDM_CHALLENGE_API_KEY to: $RANDOM_API_KEY"
...
MDM_PROXY_API_PORT=$(grep "^MDM_PROXY_API_PORT=" .env | cut -d'=' -f2)
if [ -z "$MDM_PROXY_API_PORT" ]; then
...
echo "=================================="
echo "Local endpoint: http://0.0.0.0:$MDM_PROXY_API_PORT/_web?order_id=1"
echo "Localhost endpoint: http://localhost:$MDM_PROXY_API_PORT/_web?order_id=1"
...
if [ -n "$NETWORK_IP" ]; then
    echo "Network IP: http://$NETWORK_IP:$MDM_PROXY_API_PORT/_web?order_id=1"
fi
echo ""
